...
greedy solution: fast and might be a little slow 
bruet force --> try every path 
'''
import itertools
import random
import time
...
def naive_scavenging(field):
    ...
    for path in all_paths:
        row = 0
        col = 0
        cur_gold = field[row][col]
        # figure out which direction has max total amount of gold 
        for dir in path: 
            if dir == 'ne':
                row = -1
                col = +1
        ...
        # keep track of the amount of gold we collected 
            # keep track of out of bounds 
            if col < 0 or col >= len(field) or row < 0 or row >= len(field):
                break
            cur_gold = cur_gold + field[row][col]
...
def dp_scavenging(field):
    ...
    '''
    # cache of max amount of gold we can get starting at the position 
    gold_cache = [[0] * len(field)] + len(field)
    
    #  work backwards anf ill in gold_cache 
    for col in range(len(field) -1, -1, -1): 
        for row in range(len(field)):
            # gold_cache[row][col] (max amount of gold we can get) 
            # is equal to current amount of gold at that position in the field 
            gold_at_location = field[row][col]
            # and the max of the possible gold from paths we can take from that position 
            ne_gold = 0
            se_gold = 0
            e_gold = 0
            # if we can go ne:
            if col + 1 < len(field) and row >= 0:
                ne_gold = gold_cache[row -1][col +1]
            # if we can go se:
            
            # if we can go e:
            
            # max_gold_along_possible_paths = max(ne_gold, se_gold, e_gold)
            
    max_gold = gold_cache[0]
...
print_field(tiny_field, "Tiny field")
print(naive_scavenging(tiny_field))
#
small_field = []
size = 5
for _ in range(size):
